...
import {
  ...
  X, Send, Clock, CheckCircle2, User,
} from "lucide-react";
...
function GroupDetail({ group, onClose }: GroupDetailProps) {
  const [activePanel, setActivePanel] = useState<"participants" | "email">("participants");
        ...
        <div className="flex border-b border-slate-100 flex-shrink-0">
          {([
            ...
            <button
              ...
              className={cn(
                "px-5 py-2.5 text-sm font-medium transition-colors cursor-pointer",
          ...
          {activePanel === "participants" && (
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs text-slate-400">{group.participants.length}名登録済（全{group.pax}名）</p>
                <button className="text-xs text-[#1E3A8A] hover:underline cursor-pointer">+ 参加者追加</button>
              </div>
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50 border-y border-slate-100">
                    <th className="text-left px-3 py-2 text-slate-500 font-medium">氏名</th>
                    <th className="text-left px-3 py-2 text-slate-500 font-medium">部屋番号</th>
                    <th className="text-left px-3 py-2 text-slate-500 font-medium">部屋タイプ</th>
                ...
                <tbody>
                  {group.participants.map((p) => (
                    <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                      ...
                      <td className="px-3 py-2.5 text-slate-600">{p.room_no}</td>
                      <td className="px-3 py-2.5 text-slate-500">{p.room_type}</td>
              ...
              {group.participants.length < group.pax && (
                <p className="mt-3 text-xs text-orange-500 flex items-center gap-1">
                  <span>⚠</span> 残り{group.pax - group.participants.length}名未登録
                </p>
              )}
            </div>
          )}
